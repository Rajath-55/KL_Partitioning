...
def main():
    ...
    for k in range(NO_OF_RUN):
        flag = 0
        for j in range(no_cuts):
            seed_value[j] = random.randint(0, 32767)
        # print(seed_value)
        perform_KL(sys.argv, graph, result_table)
...
def map_nodes(nodes, final_partition_core, graph):
    ...
    while True:
        ...
        if temp == best_cost[3] or temp == best_cost[2] or temp == best_cost[1] or temp == best_cost[0]:
            break
    # print("hellp")
...
def flip(G, final_partition_core, k, t, nodes, local):
    ...
    if local == 1:
        cost_arr[3] = cost_local(temp_final_partition_core[3], G, nodes, k - t, k + t-1)
    else:
